...
public class CategoryController {
	...
	public ResponseEntity<?> updateCategory(@RequestBody Category category, HttpSession session){
		...
		try {
			...
			else if(null != categoryServiceImpl.updateCategory(category, category.getCategoryId())) {
	...
	public ResponseEntity<?> getReminder(HttpSession session){
		...
		try {
			if(null != session && null != session.getAttribute("loggedInUserId")){
				...
				List<Category> categoryList = categoryServiceImpl.getAllCategoryByUserId(session.getAttribute("loggedInUserId").toString());
